...
| 15 | Final structured extraction | Regex only; GLiNER; whole-document LLM; retrieved pages + full page text to local/cloud LLM; retrieved pages + chunk + immediate neighbor chunk for non-table content, whole stitched table for table content | **Designed, not tested end-to-end.** No LLM/API was available locally; extraction accuracy is unmeasured for any option. | Retrieve top-three pages per field family, add linked table continuations (unchanged). From those pages, send the configurable LLM a narrower per-field-family context — the top-ranked chunk plus its immediate neighbor chunk, or the complete stitched table for table content (`spike/src/benchmark/context_assembly.py`) — instead of each page's full text, and let it return the complete strict candidate JSON with evidence IDs and abstention. Deterministic code validates the response; it is not the principal extractor. Selected on real-document evidence: a complete, correctly assembled stitched table took 373 characters versus 6164 for the two equivalent full pages, with all needed fields present; a stitched table also carries a per-continuation merge-confidence check so a reviewer can see which part to double-check. Sending each candidate page's full text is kept only as a documented fallback; the final choice is pending a real LLM extraction-accuracy comparison on the frozen evaluation set (see `architecture_blueprint.md`, "6. Retrieval first; the LLM creates the structured candidate JSON"). | Earlier blueprint + discussion |
